...
const fetchEntity = async (entityType: string, id: number) => {
  switch (entityType) {
    case 'User': return await prisma.user.findUnique({ where: { id } });
    case 'Order': return await prisma.order.findUnique({ where: { id } });
    case 'SupplyRequest': return await prisma.supplyRequest.findUnique({ where: { id } });
    case 'Role': return await prisma.role.findUnique({ where: { id } });
    case 'Category': return await prisma.category.findUnique({ where: { id } });
    case 'Article': return await prisma.article.findUnique({ where: { id } });
